...
region=$(jq -r '.env_id|match("ocid1\\.([a-z0-9]+)\\.([a-z0-9]+)*\\.([a-z0-9\\-]*)\\.[a-z0-9]{60}").captures[2].string' "$CONFIG_FILE")
auth="--config-file $HOME/.oci/config --profile DEFAULT --auth security_token"
if [ "${#region}" -eq 3 ]; then
  region_key_upper=$(echo "$region" | tr '[:lower:]' '[:upper:]')
  region=$(oci iam region list $auth --query "data[?key=='$region_key_upper'].name" --raw-output|jq -r ".[]")
fi
...
# -----------------------------------------------------------------
get_inventory_id() {
  echo "oci cloud-bridge inventory inventory list \
                   --compartment-id $tenant_id $auth $cb_beta"
  ...
  discovery)
    ...
    case $action in
      ...
      create-olvm)
        ...
        oci raw-request --http-method POST --target-uri "https://cloudbridge.$region.oci.oraclecloud.com/20220509/assetSources" \
        --request-body "{
          \"type\": \"OLVM\",
        ...
        case $action in
          create)
            ...
            while IFS= read -r line; do
              ...
              echo $action
              oci raw-request --http-method POST --target-uri "https://cloudmigration.$region.oci.oraclecloud.com/20220919/migrationAssets" \
              --request-body "{
